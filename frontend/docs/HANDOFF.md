...
| # | 门 | 通过标准 | 状态 |
|---|-----|---------|------|
| G1 | 环境独立可跑 | kimi 在自己会话中执行 `pnpm install` → `pnpm -r build` → `pnpm -r test` → `pnpm -r typecheck` 全绿（86 测试） | ☐ |
| G2 | 独立改码验证 | kimi 做一个微小改动（如改一条 token），流水线全绿，走完 认领→分支→提交→合并 全流程（feature/kimi-* 分支） | ☐ |
| G3 | 认领登记 | 在 TASKS.md 变更记录登记 agent_id `kimi`；认领 T-017 并创建锁文件 | ☐ |
| G4 | CI 确认 | GitHub Actions 首跑结果核对（trae 推送后触发，kimi 确认绿或登记阻塞） | ☐ |

**阻塞项登记（Blocker Register）**：

| ID | 阻塞项 | 责任人 | 状态 |
|----|--------|--------|------|
| B1 | CI 首跑尚未人工核对 | kimi（G4） | open |
...
任一 G 门未过，交接状态保持 `open`；全过后 kimi 在本文件登记 `transferred: 2026-XX-XX`。
