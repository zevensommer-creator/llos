...
| T-031 | P6b：Studio 摄入管线 + 草稿生命周期 + 沙箱试用 + 版本/下架 | todo | 待认领 | 高 | T-030 | studio/ 新包：文字输入（+PDF 端口 Fake 实现）→ 经 gateway material.generation AI 结构化 → Material Pack + DLC manifest 草稿（schema 校验，错误翻译成教学语言 §6.2）→ 表单确认（create_dlc_draft 门禁）→ 沙箱试用（编译+执行，不产生真实学习事件 §6.4）→ 发布对接 market（publish_dlc 门禁 + 告知义务确认）；版本隐形判定（素材/文案→patch/minor，教学结构→major §6.7）；market 增下架（停止新获取，已获取保留 §6.9） |
| T-032 | P6c：Studio 向导 UI + E2E 验收 | todo | 待认领 | 高 | T-031 | desktop_web Studio 向导页（粘贴/上传 → 结构化预览表单 → 沙箱试用 → 发布，校验错误教学化）+ api-client Mock studio 方法；scripts/e2e-p6.mjs 打通 BUILD_PLAN P6 验收门：非技术教师从零发布免费 DLC → 市场可获取 → 可训练 |
