...
| T-025 | P4b：市场后端 + 免费获取授权 | review | trae | 高 | T-024 | 完成待审：@llos/market 新包（9 包）；MarketService——发布门禁 publish_dlc（§2.1 能力即权威）；listing（语言/难度/自由标签/计费模型，校验：标题非空/语言 BCP-47 形/付费须正价）；query 筛选（语言主分类+难度+标签 AND+搜索标题/标签）与排序（newest/rating_desc/downloads_desc，平分比评价数）；免费获取→core entitlementStore 永久授权（无 expires），幂等（重复获取不刷 granted_at 不重复计数），resource_ref=dlc/<id> 无版本（§6.7 更新自动可达）；评价门禁已获取（§4.3），1-5 整数校验，一用户一评覆盖更新，聚合投影；付费模型 pricing_not_available（P8）；版本更新仅发布者；**core 微扩展**：entitlements.get() 只读原语（幂等分支用，锁范围已扩）；8 新测试；全 workspace 280 测试 + typecheck 绿 |
