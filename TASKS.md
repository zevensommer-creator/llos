...
| T-033 | P7a：PNG/OCR 摄入管线 | doing | trae | 高 | T-032 | 图片摄入端口：image source（PNG bytes + media_type）→ 经 gateway material.generation operation=ocr 提取文字（BYOK 优先、平台零调用）→ 复用现有 structure 管线；OCR 输出严格边界校验 + 教学化错误；BYOK descriptor 增 ocr operation + image/png 媒体类型；第一代确定性 Fake OCR transport（零网络）；BUILD_PLAN P7 验收：OCR 摄入测试通过 |
| T-034 | P7b：专家模式——自定义训练模式 + manifest 直接编辑 | todo | 待认领 | 高 | T-033 | 自定义训练模式：DLC manifest extensions 声明训练模式定义（mode_ref→闭合原语 step 序列模板，契约 §4.3 mode_ref 是 DLC 层概念必须 lower），compiler executable-lower 按模式生成 step 序列（capture_audio/capture_choice/branch 等已有原语组合），模式定义经 sha256 绑定校验；Studio 专家模式：草稿 manifest JSON 直接编辑 → schema 校验 + 编译门禁 + 教学化错误；验收：专家模式可发布自定义训练模式的 DLC |
| T-035 | P7c：模板加速器 + Studio v2 UI + E2E 验收 | todo | 待认领 | 高 | T-034 | 预设模板可选加速器（非必经入口，§6.2）：情景对话/动词配价等一键预填向导；desktop_web：PNG 上传入口 + 专家模式 manifest 编辑器 + 模板选择；api-client Mock 同步；scripts/e2e-p7.mjs P7 验收门：OCR 摄入→专家自定义训练模式 DLC 发布→市场获取→学员训练 |
