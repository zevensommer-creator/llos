...
| T-021 | P3a：发音证据管线骨架 + 质量门/弃权协议 | doing | trae | 高 | T-020 | @llos/speech 端口化管线：AudioAnalyzer/VAD/ASR/G2P/Aligner 类型化端口 + Fake 引擎（§12.5 零推理成本）；德语画像 de-DE 0.1.0（Stage 0 保守阈值）+ G2P 词典/规则；PronunciationAssessment 契约组装 + §8.3 弃权矩阵 + 确定性 + 不误拒测试；声学重引擎以端口锁协议，真实适配器后续 |
