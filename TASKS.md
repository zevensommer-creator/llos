...
| T-037 | UI-4/5 Studio 真实 API 纵向切片 | todo | 待认领 | 高 | T-036 | 新增 api/ 组合根（HttpApiClient + 真实 HTTP API 组合根，复用 Core/Studio/Gateway/Market 不复制业务规则，非破坏性 ADR）；正式 App 默认 HttpApiClient（Mock 仅 VITE_API_MODE=mock）；路由：账户/会话/Provider 列表/BYOK CRUD/模板/文字图片建草稿/草稿 CRUD/向导与专家编辑/removeTrainingModes/confirm/sandbox/publish/revision/delist/市场读取；写操作服务端重新授权；BYOK 零泄漏；网络 401/403/409/413/Provider 不可用/5xx 映射稳定 ViewModel；API integration tests（权限/属主/会话/BYOK 泄漏/专家编译/原子一致/OCR unavailable/发布市场/下架） |
