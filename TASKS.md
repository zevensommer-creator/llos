...
| T-035 | P7c：模板加速器 + Studio v2 UI + E2E 验收 | done | trae | 高 | T-034 | 已合并 main；studio/src/templates.ts：STUDIO_TEMPLATES 三预设模板（情景对话/动词配价/礼貌构式，§6.2 可选起点非必经）——仅预填摄入文本与建议值，零教学策略字段（policy/mode/claim/evidence 守卫测试）；templateById 查找；studio 3 新测试（模板预填经真实摄入管线产出可编译可沙箱草稿×3/id 查找/无教学策略）；api-client types.ts：StudioDraftView 增 expert_edited/manifest_json/training_modes_json（manifest_json 为专家 manifest 编辑器基线快照）+ StudioTemplateView + CreateStudioDraftInput 支持 image（base64 PNG OCR 摄入）+ ExpertEditStudioOutcome（saved/invalid_json/invalid_content/state_invalid/not_found）+ 4 新方法签名（listStudioTemplates/createStudioDraftFromTemplate/editTrainingModes/editManifest）；mock.ts：STUDIO_TEMPLATES_MOCK（与后端对齐）、createStudioDraft image 分支（base64→OCR 模拟→空图 ingest_empty 教学化）、#validateTrainingModes 复刻 parseTrainingModes 核心守卫（present 开场/schedule 收尾/恰一 capture 紧跟 evaluate/恰一 evaluate/feedback/claim 必须清单声明）、editManifest 门禁（dlc_id 与草稿一致不可改/display_name 非空/claims+passes 非空）、MockStudioDraft 增 dlc_id+manifest 快照（发布沿用草稿 dlc_id）、confirmStudioDraft expert 语义对齐（无向导编辑直接确认）、#applyEdit expert 锁定向导、发布/修订继承专家字段；7 新测试（api-client 57 全绿）；desktop_web StudioJourney v2：input 阶段模板卡片加速器（一键预填→review）+ PNG 上传按钮（FileReader base64→OCR 摄入→review 显示 OCR 溯源）+ review 阶段向导/专家模式双 tab（专家模式：训练模式定义 JSON 编辑器 + manifest JSON 编辑器（dlc_id 不可改提示），保存过编译门禁教学化错误，保存后向导锁定，确认并继续；修订草稿继承专家字段自动进专家视图）；styles.css 增模板卡片/代码编辑器/专家 tab 样式；scripts/e2e-p7.mjs P7 验收门 12 步（BYOK structure+ocr 装配→模板预填可编译沙箱→PNG 摄入 OCR/结构化全 BYOK 平台零调用→专家模式反例×3（非 JSON/双作答/未声明 claim 教学化）→extensions sha256 信封→向导锁定→专家沙箱丢弃式事件→发布→编译产物含自定义听写步骤序列（present/capture_text/evaluate/feedback/schedule+prompt 透传）→学员真实训练→修订继承定义 patch bump→下架拒新保旧）+ package.json e2e:p7 脚本；全 workspace 457 测试 + typecheck 绿；锁已释放 |
| T-036 | P7 专家模式与 OCR 边界加固 | review | trae | 高 | T-034/T-035 | 12 项全部落地：training-modes-parse.ts 单一浏览器安全 parser（compiler exports 子路径供 Mock 复用，去双 parser 漂移）；envelope 严格校验 schema_id/schema_version/URI/hash；drafts.ts 原子一致（editManifest 双向守卫 + removeTrainingModes 单独行为）；StudioJourney 保存后刷新 manifest_json、媒体类型如实上报+5MB 上限+FileReader 错误+ocrBusy 防重复；Provider 家族 listByokProviderFamilies 去品牌常量；capture 白名单缩为可执行五原语（audio/choice 移除）；stage_modes/未知字段/参数上下界/safe-integer/正时长（PT0S/P 拒绝）全守卫；Fake OCR 移入 test/fixtures；OCR 无 Provider→provider_unavailable typed。反例测试：compiler 44 + studio 38 + api-client 57 全绿；全 workspace build+e2e:p7 绿。待 GLM 终验 |
| T-037 | UI-4/5 Studio 真实 API 纵向切片 | review | trae | 高 | T-036 | 已实现待终验：新增 api/ 组合根（@llos/api：backend 组合根+rpc 37 方法+零依赖 node:http server+server-cli+wire 1:1 镜像+确定性平台 Provider；借鉴 MIT tinyjsonrpc-server 方法表模式）；HttpApiClient 全 40 方法（fetch+超时+ApiErrorCode 稳定映射）；正式 App 去 Mock（apiClient.ts 工厂 + VITE_API_MODE 切换 + vite /api proxy + dev:real 脚本 + .env.real）；服务端重新授权（create_dlc_draft/publish_dlc/create_class 域内门禁 + 集成测试反例）；BYOK 零泄漏断言；ADR-014；api 集成测试 10/10 + api-client build + web typecheck/vite build 绿；HTTP 冒烟（health/overview/studio/market 经 proxy 真实数据）。待 GLM 终验 |
| T-038 | P7 浏览器 E2E、CI 与验收包 | todo | 待认领 | 高 | T-037 | 保留 e2e:p7 作服务级 integration gate（修正"无 Mock/真实 OCR"文案）；e2e:p7 进 CI（feature/** push/PR 即跑）；Playwright 桌面 Web E2E（真实启动 API+Web，真实 PNG fixture，Provider 可 Fake 但代码路径经真实 HTTP API+Gateway adapter）；覆盖教师登录/模板建草稿/上传 PNG/OCR 不可用/保存训练模式后再存 manifest 不丢 extension/未知 stage 与坏 manifest 教学化拒绝/沙箱/发布/学员获取训练/修订继承/下架保旧/未验证发布 403；CI 失败存 Playwright trace；新增 pnpm verify:p7 串行 build+test+typecheck+e2e:p7+e2e:p7:web；生成 docs/reviews/P7_IMPLEMENTATION_PACKET.md（base/head SHA、commit、diff stat、命令 exit code、反例、未覆盖项、真实 OCR smoke 是否执行） |
